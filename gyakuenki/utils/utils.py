...
def frame_latest_common_time(output_frame: str, input_frame: str, buffer: tf2_ros.Buffer) -> Time:
    try:
        latest_time = buffer.get_latest_common_time(output_frame, input_frame)
    except tf2_ros.LookupException as e:
        raise ValueError(f"Error while getting latest common time: {e}, input_frame: {input_frame}, output_frame: {output_frame}")
    return latest_time
...
def transform_plane_to_frame(
    ...
    try:
        ...
        raise ValueError(f"Error while transforming points: {e}")
...
def get_point_fields(projected_objects: ProjectedObjects):
    points_list = []
